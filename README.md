...
Authenticated users can upload documents from the **Uploaded Work** page. The server stores the original file, a summary generated by the LLM, and vector embeddings generated with the `multimodal-embedding-3-small` model. Embeddings are indexed with `sqlite-vss` for cosine similarity search. The page lists each of your uploads with its summary for easy review.
