...
Authenticated users can upload documents from the new **Uploaded Work** page. The server stores the original file, generates a detailed summary of the content using `gpt-4o`, and then embeds that summary with the `text-embedding-3-small` model. Timestamps for when the work was completed and uploaded are also recorded.
