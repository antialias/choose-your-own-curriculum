...
Authenticated users can upload documents from the **Uploaded Work** page. The server stores the original file, a summary generated by the LLM, and timestamps for when the work was completed and uploaded. Embedding vectors are generated with an OpenAI model controlled by the `EMBEDDING_MODEL` environment variable (default `text-embedding-3-small`) and indexed using **sqlite-vec** for fast similarity search. The page lists each of your uploads with its summary for easy review.
...
Run `pnpm run fetch-tags-for-embeddings` to generate a list of tags across all uploaded work. The command asks the LLM for the top 100 tags, stores them in the `tag` table, fetches their embeddings using the model specified by `EMBEDDING_MODEL`, and indexes them for cosine similarity search.
