...
Authenticated users can upload images of work from the new **Uploaded Work** page. The server stores the original file, a detailed summary generated by `gpt-4o`, embeddings of that summary using the `text-embedding-3-small` model, and timestamps for when the work was completed and uploaded.
