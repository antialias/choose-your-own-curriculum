...
Authenticated users can upload documents from the **Uploaded Work** page. The server stores the original file, a summary generated by the LLM, and an embedding vector generated by the `multimodal-embedding-3-small` model. Vectors are indexed using `sqlite-vss` for fast cosine search. Timestamps for when the work was completed and uploaded are also stored. The page lists each of your uploads with its summary for easy review.
